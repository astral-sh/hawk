...
Hawk is pinned to Rust 1.98.1 and uses `rustc_private`. A prebuilt release
...
```sh
cargo +1.98.1 hawk check --manifest-path /path/to/workspace/Cargo.toml
```

## Build Hawk

Hawk is pinned to Rust 1.98.1 and uses `rustc_private`; the repository
